...
from train_yolo_26_classification.utils import custom_callbacks

# Update a setting
settings.update({'mlflow': False})


class TrainYolo26ClassificationParam(TaskParam):

    def __init__(self):
        TaskParam.__init__(self)
        dataset_folder = os.path.join(os.path.dirname(os.path.realpath(__file__)), "dataset")
        self.cfg["dataset_folder"] = dataset_folder
        self.cfg["model_name"] = "yolo26m-cls"
        self.cfg["epochs"] = 100
...
class TrainYolo26ClassificationParamFactory(dataprocess.CTaskParamFactory):
    def __init__(self):
        dataprocess.CTaskParamFactory.__init__(self)
        self.name = "train_yolo_26_classification"

    def create(self):
        return TrainYolo26ClassificationParam()


class TrainYolo26Classification(dnntrain.TrainProcess):
    ...
    def __init__(self, name, param):
        ...
        if param is None:
            self.set_param_object(TrainYolo26ClassificationParam())
        else:
    ...
    def _save_trained_weights(self, output_folder):
        try:
            trainer = getattr(self.model, "trainer", None)
            if trainer is not None and getattr(trainer, "wdir", None):
                weights_dir = Path(trainer.wdir)
            else:
                weights_dir = Path(output_folder) / "train" / "weights"

            weights_dir.mkdir(parents=True, exist_ok=True)
            self.model.save(weights_dir / "last.pt")
            self.model.save(weights_dir / "best.pt")
        except Exception as err:
            print(f"Warning: failed to save model weights: {err}")
    ...
    def run(self):
        ...
        self._save_trained_weights(output_folder)
        settings.reset()
...
class TrainYolo26ClassificationFactory(dataprocess.CTaskFactory):

    def __init__(self):
        dataprocess.CTaskFactory.__init__(self)
        self.info.name = "train_yolo_26_classification"
        ...
        self.info.repository = "https://github.com/Ikomia-hub/train_yolo_26_classification"
        self.info.original_repository = "https://github.com/ultralytics/ultralytics"
        self.info.keywords = "YOLO, YOLO26, classification, ultralytics, imagenet"
    ...
    def create(self, param=None):
        return TrainYolo26Classification(self.info.name, param)
